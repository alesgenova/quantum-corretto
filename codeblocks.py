...
class Module(Codeblock):
    ...
    def objectify(self):
        ...
        definition_file = "./{}{}_definition.f90.QC".format(directory, self.name)
        procedures_file = "./{}{}_procedures.f90.QC".format(directory, self.name)
        #print(definition_file)
        #print(procedures_file)
        procedures = ["allocate","init"]
        ...
        with open(procedures_file,"w") as f:
            ...
            f.write("subroutine allocate(this, n0)\n")
            f.write("  implicit none\n\n")
            f.write("  class({}_type), intent(inout) :: this\n".format(self.name))
            f.write("  integer, intent(in) :: n0\n\n")
            for key, var in self.declarations.items():
                if var['allocatable']: f.write("  allocate( {}({}) )\n".format(var['name'], var['dimension']))
            f.write("end subroutine allocate\n\n")
            # the init blueprint
