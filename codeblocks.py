...
class Module(Codeblock):
    ...
    def objectify(self):
        if len(self.datatypes) > 0 :
            directory = self.parent.shortdir+"datatypes/autogenerated/"
            skip_module_file = "./{}skip_modules.txt".format(directory)
            print("WARNING, the module '{}' already declares Datatypes, skipping objectification.".format(self.name))
            with open(skip_module_file, "a") as f:
                f.write(self.name+"\n")
        from default_procedures import write_alloc, write_init, write_dealloc
        directory = self.parent.shortdir+"datatypes/autogenerated/"
        ...
        with open(definition_file,"w") as f:
            ...
            f.write("end type {}_type".format(self.name)+"\n")
...
def parse_declaration(context,mo):
    if context.typ not in ["Module", "Datatype"]: return
